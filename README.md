...
| **Phase 11**: configurable rate-card providers | Real, non-mock motor quotes computed from admin-editable rate cards - AMACO and Pioneer Insurance Kenya seeded from their actual published rating guides (`app/db/rate_card_seed_data.py`). `RateCardAdapter` prices real premiums (`is_mock=False`) while binding/claims still route to staff, same as any provider without a live API yet. Full CRUD at `/api/v1/admin/rate-cards` and `/admin/rate-cards` for editing rates or onboarding a new broker with zero code changes. See `docs/RATE_CARDS.md` |
